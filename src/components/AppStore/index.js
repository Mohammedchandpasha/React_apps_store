...
class AppStore extends Component {
  ...
  getSearchFilteredApps = () => {
    const {searchInput} = this.state
    const {activeTabId} = this.state
    const tabFilteredApps = appsList.filter(
      each => each.category === activeTabId,
    )
    const filteredApps = tabFilteredApps.filter(each =>
  ...
  render() {
    ...
    const filteredApps =
      searchInput === ''
        ? this.getFilteredAppList(activeTabId)
        : this.getSearchFilteredApps(activeTabId)
